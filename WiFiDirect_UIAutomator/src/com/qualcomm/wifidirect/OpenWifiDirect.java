...
public class OpenWifiDirect extends UiAutomatorTestCase {
	...
	public void testDemo() throws UiObjectNotFoundException,
		...
		UiObject wifiOption = new UiObject(new UiSelector().className(
				"android.widget.TextView").textContains("Fi"));
		wifiOption.clickAndWaitForNewWindow();
		sleep(2000);

		UiObject moreOptions = new UiObject(new UiSelector().className(
				"android.widget.ImageButton").description("More options"));
		moreOptions.clickAndWaitForNewWindow();

		UiObject selectP2P = new UiObject(new UiSelector().className(
				"android.widget.TextView").textContains("Fi Direct"));
		UiObject isWiFiON = new UiObject(new UiSelector().className(
		...
		if (isWiFiON.isEnabled() == true) {
			...
			Log.e(LOG_TAG,
					"Couldn't Access WiFi-Direct, check if there is any issue with Device WiFi");			
		}

		UiObject P2PdeviceIDparent = new UiObject(new UiSelector().className(
				"android.widget.FrameLayout").index(1));
		UiObject P2PdeviceID = P2PdeviceIDparent.getChild(new UiSelector()
